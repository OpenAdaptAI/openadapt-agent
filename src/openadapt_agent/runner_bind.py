"""Parse-only grammar for ``openadapt://runner`` and pack URLs.

This module does not claim, store, poll, or GET. GET of ``/j/{id}`` is
the handshake and is not actuation.
"""

from __future__ import annotations

import re
from urllib.parse import parse_qs, urlparse, urlsplit, unquote

AUTHORING_ORIGIN = "https://openadapt.ai"
MAX_URI_BYTES = 2048
ALLOWED_FIELDS = frozenset({"pack", "bind", "origin"})
PACK_URL_FIELDS = frozenset({"bind", "origin"})
...
PACK_PATH_RE = re.compile(r"^/j/([^/]+)/?$")

__all__ = [
    "AUTHORING_ORIGIN",
    "MAX_URI_BYTES",
    "RunnerBindError",
    "canonical_authoring_origin",
...
def parse_connect_target(value: object) -> dict[str, str]:
    """Parse ``openadapt://runner`` or a pack URL. Does not GET."""

    if not isinstance(value, str) or not value or len(value) > MAX_URI_BYTES:
        raise RunnerBindError("Invalid OpenAdapt runner link")
    stripped = value.strip()
    if stripped.startswith("openadapt:"):
        return parse_runner_uri(stripped)
    parsed = urlsplit(stripped)
    if parsed.scheme != "https" or parsed.hostname != "openadapt.ai":
        raise RunnerBindError("Invalid OpenAdapt runner link")
    ...
    try:
        ...
        raise RunnerBindError("Invalid OpenAdapt runner link") from exc
    if port is not None or parsed.netloc != "openadapt.ai":
        raise RunnerBindError("Runner link does not name the OpenAdapt authoring origin")
    match = PACK_PATH_RE.fullmatch(parsed.path)
    if match is None:
        raise RunnerBindError("Pack URL is not a job page")
    pack = unquote(match.group(1))
    if not valid_pack_id(pack):
        raise RunnerBindError("Pack id is malformed")
    if not parsed.query:
        return {"pack": pack, "origin": AUTHORING_ORIGIN}
    try:
        query = parse_qs(parsed.query, keep_blank_values=True, strict_parsing=True)
    except ValueError as exc:
        raise RunnerBindError("Invalid OpenAdapt runner link") from exc
    if set(query) - PACK_URL_FIELDS or any(len(values) != 1 for values in query.values()):
        raise RunnerBindError("Runner link contains unknown or duplicate fields")
    origin = AUTHORING_ORIGIN
    if "origin" in query:
        origin = canonical_authoring_origin(query["origin"][0])
    result = {"pack": pack, "origin": origin}
    if "bind" in query:
        bind = query["bind"][0]
        if not valid_bind_token(bind):
            raise RunnerBindError("Bind token is malformed")
